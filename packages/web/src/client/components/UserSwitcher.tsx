...
export default function UserSwitcher({ users, currentUserId, onChange }: Props) {
  ...
  return (
    <div className="inline-flex rounded-xl bg-slate-100 dark:bg-slate-800 p-1" role="tablist" aria-label="ユーザー切替">
      {users.map((u, idx) => {
        ...
        return (
          <button
            ...
            className={`min-h-10 px-3 rounded-lg text-sm font-medium flex items-center gap-1.5 transition-colors ${
              isActive ? `${theme.active} shadow-sm` : 'text-slate-500 dark:text-slate-400'
            }`}
          >
