...
## expense-field-order.spec.ts

明細入力ダイアログの「説明」「金額」の並び順。ドラッグで上下を入れ替えられ、その並びがユーザごとに保存され、ページを開き直しても保たれることの確認。
...
CLI（`packages/cli`）で支出を追加・一覧できることの確認。CLI は `npx tsx packages/cli/src/index.ts` を子プロセスで実行し（`WEB_URL=http://localhost:3121`）、事前ビルド不要にしてある。

CLI にとってのUIは標準出力・標準エラーなので、そこに表示される文言を検証する。**加えて「どちらの表に入ったか」「精算にどう効いたか」は画面で確認する**（CLIの出力だけでは、正しいユーザ・正しい月に入ったことをユーザ視点で保証できない）。
