...
CLI にとってのUIは標準出力・標準エラーなので、そこに表示される文言を検証する。**加えて「どちらの表に入ったか」「精算にどう効いたか」は画面で確認する**（CLIの出力だけでは、正しいユーザ・正しい月に入ったことをユーザ視点で保証できない）。コマンドの出力と画面への反映は別の観点なのでテストを分ける。

「指定していないユーザ／月の明細には表示されない」を検証するときは、先に空状態の文言（`今月の支出はまだありません`）を `waitFor()` して明細の読み込みを待つ。待たずに `toBeHidden()` すると、読み込み前の状態を見て空振りで通ってしまう。
